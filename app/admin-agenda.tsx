...
import { Border, Color, FontFamily, FontSize, Gap, LineHeight, Padding, StyleVariable } from "../GlobalStyles";
...
const clientLabel = (appointment: AppointmentResponse, names: Record<number, string>) =>
...
const AdminAgendaScreen = () => {
  ...
  const loadAppointments = useCallback(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const uniqueIds = Array.from(new Set(items.map((item) => item.clientId).filter(Boolean)));
        const missingIds = uniqueIds.filter((id) => !clientNames[id]);

        if (missingIds.length) {
          const responses = await Promise.allSettled(missingIds.map((id) => getUserById(id)));
          if (!isActive) {
            return;
          }
        ...
        Alert.alert("Nao foi possivel carregar", "Tente novamente em instantes.");
      } finally {
        if (isActive) {
          setIsLoading(false);
        }
  ...
  const appointmentsForDay = useMemo(() => {
    return appointments
      ...
      .filter((item) => {
        ...
        return Number.isNaN(when.getTime()) ? false : sameDay(when, selectedDate);
      })
  ...
  const handleOpenOptions = (appointment: AppointmentResponse) => {
    ...
    Alert.alert(title, "Escolha uma opcao", [
      ...
      },
      {
        ...
        style: "destructive",
        onPress: () => {
          Alert.alert("Confirmar cancelamento", "Essa acao nao pode ser desfeita.", [
  ...
  const renderDay = (value: number | null, index: number) => {
    ...
    const isToday = sameDay(new Date(), new Date(anchorMonth.getFullYear(), anchorMonth.getMonth(), value));
    return (
      <TouchableOpacity
        key={`day-${value}`}
        style={[styles.dayCell, isSelected && styles.dayCellActive]}
        onPress={() => setSelectedDate(new Date(anchorMonth.getFullYear(), anchorMonth.getMonth(), value))}
        activeOpacity={0.85}
      >
        <Text style={[styles.dayText, isSelected && styles.dayTextActive]}>{value}</Text>
  ...
  const renderScheduleItem = ({ item }: { item: AppointmentResponse }) => {
    const when = new Date(item.scheduledAt);
    const time = Number.isNaN(when.getTime()) ? "--:--" : when.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });
    return (
      <View style={styles.slotCard}>
        <View style={styles.slotHeader}>
          <View style={styles.avatar} />
          <View style={styles.slotTitleArea}>
            <Text style={styles.slotName}>{clientLabel(item, clientNames)}</Text>
          </View>
          <TouchableOpacity onPress={() => handleOpenOptions(item)} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
            <Ionicons name="ellipsis-vertical" size={18} color={Color.mainTrunks} />
          </TouchableOpacity>
        </View>
        <View style={styles.slotDivider} />
        <View style={styles.slotFooter}>
          <Text style={styles.slotTime}>{time}</Text>
          <Text style={styles.slotService}>{item.serviceType ?? "Atendimento"}</Text>
        </View>
      </View>
    );
  };
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.nextCard}>
            {isLoading ? (
              ...
              <>
                <Text style={styles.nextClient}>{`${clientLabel(nextAppointment, clientNames)} e o seu proximo cliente`}</Text>
                <Text style={styles.nextDate}>{formatDateLong(nextAppointment.scheduledAt)}</Text>
                <TouchableOpacity
                ...
                >
                  <Text style={styles.linkText}>Ver detalhes do agendamento</Text>
              ...
              <>
                ...
                <Text style={styles.nextDate}>Selecione um dia para ver os horarios.</Text>
              </>
            )}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {isLoading ? (
            ...
            <View style={styles.listEmpty}>
              <Text style={styles.emptyText}>Nenhum horario para esta data.</Text>
            </View>
          ) : (
            <FlatList
